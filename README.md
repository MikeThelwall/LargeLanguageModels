...
If the Jsonl batch file created in the first step then you can run it with the above program, after entering the location of the batch files and the number of iterations (repeated submissions of the same batch file). You can list multiple batch files for it to submit in one go. It runs them sequentially in series, not parallel, in case of quota issues.
